...
const StudentDashboardRedesigned = ({ user }) => {
    ...
    const [integrations, setIntegrations] = useState({ google: false, microsoft: false });
    ...
    useEffect(() => {
        ...
        } else if (status === 'microsoft_connected') {
            toast.success('Successfully linked Microsoft To-Do!');
            window.history.replaceState({}, document.title, window.location.pathname);
        }
    }, []);
    ...
    const handleGenerateAttendanceCert = () => {
        ...
        const date = new Date().toLocaleDateString('en-GB');
        const totalAtt = (data?.attendance?.present || 0) + (data?.attendance?.absent || 0) + (data?.attendance?.late || 0);
        const attendanceRate = totalAtt > 0 ? Math.round(((data.attendance.present || 0) / totalAtt) * 100) : 0;
        const win = window.open('', '_blank');
        win.document.write(`<html><head><title>Attendance Certificate</title><style>body{font-family:Georgia,serif;padding:60px;max-width:700px;margin:0 auto}.cert{border:8px double #b45309;padding:40px;text-align:center}.title{font-size:28px;font-weight:bold;letter-spacing:3px;margin:20px 0}.name{font-size:32px;font-weight:bold;color:#b45309;border-bottom:2px solid #b45309;display:inline-block;padding:0 20px}.rate{font-size:52px;font-weight:bold;color:#16a34a;margin:20px 0}.sigs{display:flex;justify-content:space-around;margin-top:60px}.sig{border-top:1px solid #000;width:180px;padding-top:10px;font-size:13px}@media print{body{padding:20px}}</style></head><body><div class="cert"><h2>MATUNDU PRIMARY SCHOOL</h2><div class="title">CERTIFICATE OF ATTENDANCE</div><p>This is to certify that</p><div class="name">${student.firstName} ${student.lastName}</div><p>Adm: <strong>${student.admissionNumber}</strong> | Class: <strong>${student.class?.name || 'N/A'}</strong></p><p>Has achieved an attendance rate of</p><div class="rate">${attendanceRate}%</div><p>for Academic Year 2026</p><div class="sigs"><div class="sig">Class Teacher</div><div class="sig">Headteacher</div></div><p style="margin-top:30px;font-size:11px;color:#666">Issued: ${date} | Cert No: ATT-${Date.now().toString().slice(-6)}</p></div><script>window.onload=function(){window.print()}<\/script></body></html>`);
        win.document.close();
    ...
    const IntegrationsCard = () => (
        <div className="bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50 mt-8">
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-gray-800">Google Calendar</p>
                            <p className="text-[8px] text-gray-400 font-bold uppercase tracking-tight">
                                {integrations.google ? 'Connected' : 'Not Linked'}
                            </p>
                        </div>
                    </div>
                    {!integrations.google && (
                        <button 
                            ...
                            className="text-[10px] font-bold text-white bg-teal-600 px-3 py-1.5 rounded-xl hover:bg-teal-700 transition"
                        >
                            Link
                        </button>
                    )}
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-gray-800">Microsoft To-Do</p>
                            <p className="text-[8px] text-gray-400 font-bold uppercase tracking-tight">
                                {integrations.microsoft ? 'Connected' : 'Not Linked'}
                            </p>
                        </div>
                    </div>
                    {!integrations.microsoft && (
                        <button 
                            ...
                            className="text-[10px] font-bold text-white bg-[#476C63] px-3 py-1.5 rounded-xl hover:bg-[#39564f] transition"
                        >
                            Link
                        </button>
                    )}
            ...
            <p className="text-[8px] text-gray-400 mt-4 leading-tight">
                Connect your personal accounts to automatically sync school reminders to your phone calendar and task lists.
            </p>
        </div>
    );
                                ...
                                announcements.map((ann, idx) => (
                                    <div key={ann.id} className="space-y-4 animate-in slide-in-from-right duration-500 delay-150 border-b border-gray-50 pb-6 last:border-0 last:pb-0">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 bg-[#476C63] rounded-full flex items-center justify-center text-white font-bold text-xs shadow-lg shadow-teal-100">MA</div>
                                                <div>
                                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">
                                                        {new Date(ann.publishedAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                                                    </p>
                                                    <p className="text-[11px] font-bold text-gray-800">Matundu Institute</p>
                                                </div>
                                            </div>
